...
bool isBipartiteUtil(vector<vector<bool>> G, int src, vector<int> &colorArr) {
    int n = G.size();
    colorArr[src] = 1;
...
bool isBipartite(vector<vector<bool>> G, vector<int> &colorArr) {
...
int main() {
    ...
    vector<vector<bool>> GC(n, vector<bool>(n, 0));
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
            GC[i][j] = (i != j) ? !G[i][j] : 0;

    vector<int> colorArr(n, -1);
    bool ans = isBipartite(GC, colorArr);
    if (ans) {
        int k = 0;
        for (int c: colorArr) {
            if (c == 1) {
