...
public:
    ...
    bool addPacket(int source, int destination, int timestamp) {
        ...
        if (!check) {
            ...
            if (this->count >= this->capacity) {
                ...
                int currSource = v[0], currDest = v[1], currTime = v[2];
                this->sources.erase(currSource);
                this->dests.erase(currDest);
                this->tms.erase(currTime);
                --this->mp[destination][currTime];
                this->dq.pop_front();
                --this->count;
            }
            this->sources.insert(source);
            this->dests.insert(destination);
            this->tms.insert(timestamp);
    ...
    vector<int> forwardPacket() {
        ...
        int currSource = answer[0], currDest = answer[1], currTime = answer[2];
        this->sources.erase(currSource);
        this->dests.erase(currDest);
        this->tms.erase(currTime);

        --this->mp[currDest][currTime];
