...
class Solution {
public:
    void dfs(TreeNode *cur, int x, int y, int &depth, int &ansX, int &ansY, TreeNode *&parentX, TreeNode *&parentY) {
        if (!cur) {
            return;
        }
        int val;
        if (cur->left) {
            ...
            ++depth;
            dfs(cur->left, x, y, depth, ansX, ansY, parentX, parentY);
            --depth;
        }
        if (cur->right) {
            val = cur->right->val;
            ...
            ++depth;
            dfs(cur->right, x, y, depth, ansX, ansY, parentX, parentY);
            --depth;
        }
    }
    ...
    bool isCousins(TreeNode *root, int x, int y) {
        ...
        int ansX, ansY;
        dfs(root, x, y, depthX, ansX, ansY, parentX, parentY);
...
//    root = t4;
    ...
    TreeNode *t2 = new TreeNode(4, nullptr, t1);
    TreeNode *t3 = new TreeNode(3, nullptr, t2);
